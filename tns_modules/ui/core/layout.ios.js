...
var LayoutInfo = (function () {
    ...
    LayoutInfo.prototype.arrange = function (finalRect) {
        ...
        if (this.arrangeDirty || this.neverArranged || !geometry.Rect.equals(this.finalRect, finalRect)) {
            var firstArrange = this.neverArranged;
...
var LayoutManager = (function () {
    ...
    LayoutManager.prototype.markTreeDirtyHelper = function (element) {
        ...
        if (element.view.children) {
            var children = element.view.children;
            var childrenCount = children.length;
            for (var i = 0; i < childrenCount; i++) {
                var child = children[i];
                this.markTreeDirtyHelper(child._layoutInfo);
            }
        }
    };
                ...
                while (true) {
                    if (++loopCounter > 153) {
                        ...
                        if (!loopStartTime) {
                            ...
                            var diff = loopStartTime.timeIntervalSinceNow * -1000;
                            if (diff > 2 * 153) {
                                timer.setTimeout(function () {
                                    _this.needsRecalc();
                                }, 20);
